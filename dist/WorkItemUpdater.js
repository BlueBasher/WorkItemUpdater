        ...
        try {
            ...
            if (!workItemRefs || workItemRefs.length === 0) {
                ...
                yield asyncForEach(workItemRefs, (workItemRef) => __awaiter(this, void 0, void 0, function* () {
                    ...
                    if (yield updateWorkItem(workItemTrackingClient, workItem, settings)) {
...
function getVstsWebApi() {
    ...
    const credentialHandler = (0, WebApi_1.getHandlerFromToken)(accessToken);
...
function getWorkItemsRefs(vstsWebApi, workItemTrackingClient, settings) {
    return __awaiter(this, void 0, void 0, function* () {
        if (settings.workitemsSource === 'Build') {
            ...
            const workItemRefs = yield buildClient.getBuildWorkItemsRefs(settings.projectId, settings.buildId, settings.workitemLimit);
            return workItemRefs;
        }
        ...
        if (settings.workItemType.split(',').indexOf(workItem.fields['System.WorkItemType']) >= 0) {
            ...
            if (settings.addTags || settings.removeTags) {
                ...
                if (removeTags.length > 0)
                    operation = VSSInterfaces_1.Operation.Replace;
            ...
            if (settings.updateFields) {
                const updateFields = settings.updateFields.split(/\r?\n/);
                updateFields.forEach((updateField) => {
                    ...
                    if (commaIndex >= 0) {
                        ...
                        const workItemField = workItem.fields[fieldName];
                        if (workItemField) {
                            if (workItemField.type === WorkItemTrackingInterfaces_1.FieldType.DateTime) {
            ...
            if (document.length > 0) {
                const updatedWorkItem = yield workItemTrackingClient.updateWorkItem(undefined, document, workItem.id, undefined, false, settings.bypassRules);
                console.log('WorkItem ' + workItem.id + ' updated');
            }
            if (settings.comment) {
                const addCommentToWorkItem = yield workItemTrackingClient.addComment({ text: settings.comment }, settings.projectId, workItem.id);
